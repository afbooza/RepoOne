...
.controller('mediaController',['$scope', '$http', function($scope, $http){
	...
	.controller('gameController',['$scope', '$http', function($scope, $http) {
		//location.reload();
			...
			function yRandom() {
				return Math.floor(Math.random() * (460 + yStart)) + yStart
			}

			function xRandom() {
				return Math.floor(Math.random() * (920 + xStart - 50)) + xStart + 50
			}
			...
			var background =
			{
				draw: function () {
					...
					bgImage.src = "img/field_notext.png";
				}
			}
		...
		function drawElapsedTime() {
			...
			var formatted = convertTime(diff);
			var t = setTimeout(drawElapsedTime, 500);
			if (diff < 120000) {
				ctx.fillText("Clock: " + formatted, 200, 32);
			}
			else {
				score = 0;
				initial = Date.now();
